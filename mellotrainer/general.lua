...
Citizen.CreateThread( function()
	while true do
		...
		if ( IsControlJustReleased( 1, 113 ) or IsDisabledControlJustReleased( 1, 113 ) ) and not blockinput and ((settings["adminOnlyTrainer"] == true and adminStatus == true) or settings["adminOnlyTrainer"] == false) then -- f6
		...
		if IsControlJustReleased(1, 113) and not blockinput and ((settings["adminOnlyTrainer"] == true and adminStatus == true) or settings["adminOnlyTrainer"] == false) then -- f3
			teleportToWaypoint()
		end

		if showtrainer and not blockinput then
			if ( IsControlJustPressed( 1, 112 ) or IsControlJustPressed( 1, 112 ) ) then
				showtrainer = false
				SendNUIMessage({
					hidetrainer = true
				})
